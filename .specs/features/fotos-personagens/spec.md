...
| Fonte TMDB (round 2) | Removida do catálogo em 2026-08-05, junto do aviso de não-endosso na interface | O Verifier reprovou IMG-02 para as 41 entradas: TMDB devolve o `profile_path` do ator/atriz, a foto genérica de divulgação, não uma still do personagem no papel. Evidência: (1) três pares de personagens diferentes com URL idêntica porque o ator é o mesmo — `neo`/`john wick` (Keanu Reeves), `jack sparrow`/`willy wonka` (Johnny Depp), `michael corleone`/`tony montana` (Al Pacino); (2) `obi wan kenobi` era um retrato preto e branco de Alec Guinness dos anos 1950, de terno, sem contexto de papel; (3) headshot de divulgação sem figurino nas 41 entradas. Decisão do dono do projeto: qualidade acima de cobertura — catálogo cai de 179 (4 fontes) para 138 (3 fontes: Wikimedia Commons, AniList, Comic Vine). `tests/wordlist.test.ts` tira 'TMDB' de `knownSources` e do mapa host-por-fonte, e ganha um teste dedicado que falha se qualquer entrada apontar para `image.tmdb.org` — reintroduzir a fonte quebra a suíte de propósito | y |
